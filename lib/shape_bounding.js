import CurveBounding from "./curve_bounding";
...
function boundingRectOfEllipse(ellipse, shouldReturnTrueBounding) {
  ...
  if (transform) {
    ...
    if (shouldReturnTrueBounding) {
      ...
      var D =
        2 * ry * ry * (mc * md * mf - md * md * me) + 2 * rx * rx * (ma * mb * mf - mb * mb * me) - 2 * (cx * ry * ry * md - cy * rx * rx * mb) * denominator;
      var E =
        2 * ry * ry * (mc * md * me - mc * mc * mf) + 2 * rx * rx * (ma * mb * me - ma * ma * mf) + 2 * (cx * ry * ry * mc - cy * rx * rx * ma) * denominator;
      var F =
        ry * ry * (mc * mc * mf * mf - 2 * mc * md * me * mf + md * md * me * me) +
        rx * rx * (ma * ma * mf * mf - 2 * ma * mb * me * mf + mb * mb * me * me) +
        (2 * cx * ry * ry * (md * me - mc * mf) + 2 * cy * rx * rx * (ma * mf - mb * me)) * denominator +
        (ry * ry * cx * cx + rx * rx * cy * cy - rx * rx * ry * ry) * Math.pow(denominator, 2);
      var a = 4 * A * C - B * B;
      var b1 = 4 * A * E - 2 * B * D;
      var c1 = 4 * A * F - D * D;
      var d1 = b1 * b1 - 4 * a * c1;
...
function boundingRectOfPath(path, shouldReturnTrueBounding) {
  ...
  if ((path.transform && shouldReturnTrueBounding) || /[Aa]/.test(d)) {
  ...
  params = d.replace(/[mlhvaqtcsz]/gi, "#").split("#");
  params.shift();
  params.forEach(function (str, idx) {
    ...
    if (/[ML]/.test(command)) {
      ...
      for (i = 0; i < args.length; i += 6) {
        if (shouldReturnTrueBounding) {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, args[i], args[i + 1], args[i + 2], args[i + 3], args[i + 4], args[i + 5]);
      ...
      if (shouldReturnTrueBounding) {
        if (/[cs]/i.test(commands[idx - 1])) {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, potentialCp[0], potentialCp[1], args[0], args[1], args[2], args[3]);
        } else {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, x, y, args[0], args[1], args[2], args[3]);
        }
        checkX(trueBounds[0]); // MIN_X
        checkX(trueBounds[4]); // MAX_X
        checkY(trueBounds[1]); // MIN_Y
      ...
      if (shouldReturnTrueBounding) {
        if (/[cs]/i.test(commands[idx - 1])) {
          ...
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, x, y, x + args[0], y + args[1], x + args[2], y + args[3]);
        }
        checkX(trueBounds[0]); // MIN_X
        checkX(trueBounds[4]); // MAX_X
        checkY(trueBounds[1]); // MIN_Y
      ...
      for (i = 0; i < args.length; i += 4) {
        ...
        if (shouldReturnTrueBounding) {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, cpx1, cpy1, cpx2, cpy2, args[i + 2], args[i + 3]);
          checkX(trueBounds[0]); // MIN_X
          checkX(trueBounds[4]); // MAX_X
          checkY(trueBounds[1]); // MIN_Y
          checkY(trueBounds[5]); // MAX_Y
      ...
      for (i = 0; i < args.length; i += 4) {
        ...
        if (shouldReturnTrueBounding) {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, cpx1, cpy1, cpx2, cpy2, x + args[i + 2], y + args[i + 3]);
          checkX(trueBounds[0]); // MIN_X
          checkX(trueBounds[4]); // MAX_X
          checkY(trueBounds[1]); // MIN_Y
          checkY(trueBounds[5]); // MAX_Y
      ...
      if (shouldReturnTrueBounding) {
        trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, cpx1, cpy1, cpx2, cpy2, args[0], args[1]);
        checkX(trueBounds[0]); // MIN_X
        checkX(trueBounds[4]); // MAX_X
        checkY(trueBounds[1]); // MIN_Y
        checkY(trueBounds[5]); // MAX_Y
      ...
      for (i = 2; i < args.length; i += 2) {
        ...
        if (shouldReturnTrueBounding) {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, cpx1, cpy1, cpx2, cpy2, args[i], args[i + 1]);
          checkX(trueBounds[0]); // MIN_X
          checkX(trueBounds[4]); // MAX_X
          checkY(trueBounds[1]); // MIN_Y
          checkY(trueBounds[5]); // MAX_Y
      ...
      if (shouldReturnTrueBounding) {
        trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, cpx1, cpy1, cpx2, cpy2, x + args[0], y + args[1]);
        checkX(trueBounds[0]); // MIN_X
        checkX(trueBounds[4]); // MAX_X
        checkY(trueBounds[1]); // MIN_Y
        checkY(trueBounds[5]); // MAX_Y
      ...
      for (i = 2; i < args.length; i += 2) {
        ...
        if (shouldReturnTrueBounding) {
          trueBounds = CurveBounding.calculate(CurveBounding.Mode.STANDARD, x, y, cpx1, cpy1, cpx2, cpy2, x + args[i], y + args[i + 1]);
          checkX(trueBounds[0]); // MIN_X
          checkX(trueBounds[4]); // MAX_X
          checkY(trueBounds[1]); // MIN_Y
          checkY(trueBounds[5]); // MAX_Y
