...
function isSVGElement(object) {
  return typeof SVGElement === "object"
...
function attributesFromSVGElement(element, names) {
  ...
  names.forEach(function (name) {
    ...
    if (name === "transform" || name === "gradientTransform") {
      if (value) {
        value = Helper.normalizeTransform(value);
      }
    }
...
function textObject(textEl) {
  ...
  else if (isCheerioObject(textEl)) {
  } // do nothing
  else return null;
  ...
  if (isSVGEl) {
    Array.prototype.forEach.call(children, function (childEl) {
      ...
      obj.text = childEl.textContent;
      attributes.children.push(obj);
    });
  } else {
    children.each(function (i) {
      var $child = children.eq(i);
      if (type($child) !== "tspan") return;
      var obj = attributesFromElement($child, ["x", "y", "font-size", "letter-spacing"]);
      obj.text = $child.text();
      attributes.children.push(obj);
    });
  }
